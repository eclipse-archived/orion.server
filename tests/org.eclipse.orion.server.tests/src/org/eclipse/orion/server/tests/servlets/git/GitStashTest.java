...
public class GitStashTest extends GitTest {
	...
	public void testStashIllegalOperationType() throws Exception {
		...
		body.put(GitConstants.KEY_STASH_POST_OPERATION_TYPE, "unknown");
		WebRequest request = new PostMethodWebRequest(location, IOUtilities.toInputStream(body.toString()), "UTF-8");
	...
	static WebRequest getStashDeleteRequest(String location, int stashRef) throws Exception {
		...
		sb.append(location);
		sb.append("?"); //$NON-NLS-1$
		sb.append(GitConstants.KEY_STASH_DROP_REF);
		sb.append("="); //$NON-NLS-1$
		sb.append(stashRef);

		String requestURI = toAbsoluteURI(sb.toString());
		JSONObject body = new JSONObject();
	...
	private WebResponse stashList(JSONObject project, int page, int pageSize) throws Exception {
		...
		StringBuilder sb = new StringBuilder().append(project.getJSONObject(GitConstants.KEY_GIT).getString(GitConstants.KEY_STASH));
