...
public class GitStashTest extends GitTest {
	...
	public void testStashList() throws Exception {
		...
		JSONArray result = responseObject.getJSONArray(ProtocolConstants.KEY_CHILDREN);
	...
	public void testStashListNoNextLocation() throws Exception {
		...
		WebResponse response = stashList(project);
		JSONObject responseObject = new JSONObject(response.getText());
		JSONArray result = new JSONArray(responseObject.get(ProtocolConstants.KEY_CHILDREN));

		assertEquals(HttpURLConnection.HTTP_OK, response.getResponseCode());
		assertEquals(2, result.length());

		responseObject.getString(ProtocolConstants.KEY_NEXT_LOCATION);
	}
	...
	public void testStashListPaginated() throws Exception {
		...
		JSONArray result = resultObject.getJSONArray(ProtocolConstants.KEY_CHILDREN);

		assertEquals(HttpURLConnection.HTTP_OK, response.getResponseCode());
		assertEquals(10, result.length());
		assertTrue(resultObject.has(ProtocolConstants.KEY_NEXT_LOCATION));
		String actualLocationString = resultObject.getString(ProtocolConstants.KEY_NEXT_LOCATION);
		assertNextLocation(2, 10, actualLocationString);

		response = stashList(project, 2, 10);
		resultObject = new JSONObject(response.getText());
		result = resultObject.getJSONArray(ProtocolConstants.KEY_CHILDREN);

		assertEquals(HttpURLConnection.HTTP_OK, response.getResponseCode());
		assertEquals(2, result.length());
		assertFalse(resultObject.has(ProtocolConstants.KEY_NEXT_LOCATION));
		...
		assertEquals(10, result.length());
		assertTrue(resultObject.has(ProtocolConstants.KEY_NEXT_LOCATION));
		actualLocationString = resultObject.getString(ProtocolConstants.KEY_NEXT_LOCATION);
		assertNextLocation(2, 10, actualLocationString);
		...
		response = stashList(project, 3, 10);
		resultObject = new JSONObject(response.getText());
		result = resultObject.getJSONArray(ProtocolConstants.KEY_CHILDREN);
	...
	private void assertNextLocation(int expectedPage, int expectedSize, String actualLocationString) {

		int parameterSep = actualLocationString.indexOf('?');

		if (parameterSep == -1) {
			throw new AssertionError(GitConstants.STASH_ILLEGAL_NEXT_LOCATION_FORMAT_MESSAGE);
		}

		String baseLocation = actualLocationString.substring(0, parameterSep);
		String expectedParameterString = GitUtils.constructStashNextLocationString(baseLocation, expectedPage, expectedSize);

		assertEquals(expectedParameterString, actualLocationString);
	}
	...
	static WebRequest getStashPostRequest(String location, String opType) throws Exception {
		...
		if (opType != null) {
			body.put(GitConstants.KEY_STASH_POST_OPERATION_TYPE, opType);
		}
	...
	private WebResponse stashListAll(JSONObject project) throws Exception {

		String location = project.getJSONObject(GitConstants.KEY_GIT).getString(GitConstants.KEY_STASH);
		WebRequest request = GitStashTest.getStashListGetRequest(location);

		request.setParameter(GitConstants.KEY_STASH_LIST_PAGE, Integer.toString(0));
