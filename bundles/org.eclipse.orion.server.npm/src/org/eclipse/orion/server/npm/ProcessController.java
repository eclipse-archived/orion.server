...
public class ProcessController {
	...
	public int execute() throws InterruptedException, IOException, TimeOutException {
		...
		// starts the process
		process = Runtime.getRuntime().exec(params, null, cwd);
		Thread threadOut = null;
		Thread threadErr = null;
		if (forwardStdErr != null)
			threadErr = forwardStream("stderr", process.getErrorStream(), forwardStdErr);
		if (forwardStdOut != null)
			threadOut = forwardStream("stdout", process.getInputStream(), forwardStdOut);
	...
	private Thread forwardStream(final String name, final InputStream in, final OutputStream out) {
