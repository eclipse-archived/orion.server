				...
				if (version == 6) {
					...
					for (ManifestParseTree service : services.getChildren()) {
						...
						if (serviceInstanceGUID == null) {
							...
							error.put("error_code", "Service not found");
							error.put("service", nameService);
							status.add(new ServerStatus(IStatus.ERROR, HttpServletResponse.SC_BAD_REQUEST, "Service instance " + nameService + " cannot be found in target space", error, null));
							return status;
						}
