...
public class ProcfileUtils {
	 ...
	 * @throws IOException
	 */
	public static IProcfileEntry[] parseProcfile(IFileStore store) throws CoreException, IOException {
	...
	/**
	 * Parses a Procfile from the containing folder if possible.
	 * @param parentFolder
	 * @returns null if the Procfile does not exist, otherwise returns the entries.
	 * @throws IOExceptin if the Procfile is a directory
	 */
	public static IProcfileEntry[] readProcfileFromFolder(IFileStore parentFolder) throws CoreException, IOException {
		IFileStore procfileStore = parentFolder.getChild(CFNodeJSConstants.PROCFILE_FILE_NAME);
		IFileInfo procfileInfo = procfileStore.fetchInfo();
		if (!procfileInfo.exists())
			return null;
		return parseProcfile(procfileStore);
	}
}
