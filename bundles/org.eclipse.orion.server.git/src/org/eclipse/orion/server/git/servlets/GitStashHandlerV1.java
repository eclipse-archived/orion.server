...
public class GitStashHandlerV1 extends AbstractGitHandler {
	...
	protected boolean handleGet(RequestInfo requestInfo) throws ServletException {
		...
		try {
			...
			int pageNo = request.getParameter(GitConstants.KEY_STASH_LIST_PAGE) != null ? new Integer(request.getParameter(GitConstants.KEY_STASH_LIST_PAGE)).intValue() : 1; //$NON-NLS-1$ //$NON-NLS-2$
			int pageSize = request.getParameter(GitConstants.KEY_STASH_LIST_PAGE_SIZE) != null ? new Integer(request.getParameter(GitConstants.KEY_STASH_LIST_PAGE_SIZE)).intValue() : GitConstants.STASH_DEFAULT_PAGE_SIZE; //$NON-NLS-1$ //$NON-NLS-2$
			...
			for (RevCommit revCommit : stashedRefsList) {
				stashCommits.add(new Commit(/* not neeeded */null, db, revCommit, /* not needed */null));
			}
