...
import org.eclipse.orion.server.git.PaginatedResult;
...
public class GitStashHandlerV1 extends AbstractGitHandler {
	...
	protected boolean handlePost(RequestInfo requestInfo) throws ServletException {
		...
		try {
			...
			if (operationType == null) {
				return statusHandler.handleRequest(request, response, new ServerStatus(IStatus.ERROR, HttpServletResponse.SC_BAD_REQUEST, GitConstants.STASH_MISSING_OPERATION_TYPE, /* not needed */null));
			}
	...
	protected boolean handleGet(RequestInfo requestInfo) throws ServletException {
		...
		try {
			...
			PaginatedResult paginatedResult = GitUtils.paginate(stashCommits, pageNo, pageSize, baseLocation);

			JSONObject result = new JSONObject();
			result.put(ProtocolConstants.KEY_CHILDREN, paginatedResult.getChildren());

			if (paginatedResult.hasNext()) {
				result.put(ProtocolConstants.KEY_NEXT_LOCATION, GitUtils.constructStashNextLocationString(baseLocation.toString(), pageNo + 1, pageSize));
			}
	...
	protected boolean handleDelete(RequestInfo requestInfo) throws ServletException {
		...
		try {
			...
			int dropStashRef = request.getParameter(GitConstants.KEY_STASH_DROP_REF) != null ? Integer.parseInt(request.getParameter(GitConstants.KEY_STASH_DROP_REF)) : 0;
