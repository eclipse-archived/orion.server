...
public class Clone {
	...
	private JSONArray getChildren() throws URISyntaxException, IOException, CoreException, JSONException {
        ...
        if (gitModule.exists() && !gitModule.isDirectory()) {
            ...
            while (walk.next()) {
                ...
                JSONObject submoduleCloneJSON = new Clone().toJSON(path.append(walk.getPath()).addTrailingSeparator(), baseLocation, getCloneUrlHelper(submoduleFile),newParents);
	...
	private String getCloneUrlHelper(File gitDir) {
		Repository db = null;
		try {
