...
public class GitCloneHandlerV1 extends ServletResourceHandler<String> {
	...
	public static void doConfigureClone(Git git, String user, String gitUserName, String gitUserMail) throws IOException, CoreException {
		...
		/*
		 * IOrionUserProfileNode userNode = UserServiceHelper.getDefault().getUserProfileService().getUserProfileNode(user,
		 * true).getUserProfileNode(IOrionUserProfileConstants.GENERAL_PROFILE_PART); if (userNode != null && userNode.get(GitConstants.KEY_NAME, null) != null)
		 * config.setString(ConfigConstants.CONFIG_USER_SECTION, null, ConfigConstants.CONFIG_KEY_NAME, userNode.get(GitConstants.KEY_NAME, null)); if (userNode
		 * != null && userNode.get(GitConstants.KEY_MAIL, null) != null) config.setString(ConfigConstants.CONFIG_USER_SECTION, null,
		 * ConfigConstants.CONFIG_KEY_EMAIL, userNode.get(GitConstants.KEY_MAIL, null));
		 */
