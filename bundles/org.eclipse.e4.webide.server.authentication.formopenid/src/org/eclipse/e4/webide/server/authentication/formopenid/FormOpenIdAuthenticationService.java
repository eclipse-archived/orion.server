...
public class FormOpenIdAuthenticationService implements IAuthenticationService {
	...
	public void configure(Properties properties) {
		try {
			...
			LogHelper.log(new Status(IStatus.WARNING,
					Activator.PI_FORMOPENID_SERVLETS,
					"Reconfiguring FormOpenIdAuthenticationService"));
			...
			try {
				...
				LogHelper.log(new Status(IStatus.ERROR,
						Activator.PI_FORMOPENID_SERVLETS, 1,
				...
				LogHelper
						.log(new Status(
								...
								Activator.PI_FORMOPENID_SERVLETS,
								1,
								"FormOpenIdAuthenticationService could not be configured",
								e1));
			}
	...
	private void setNotAuthenticated(HttpServletRequest req,
		...
		RequestDispatcher rd = req
				.getRequestDispatcher("/mixlogin/login?redirect=" //$NON-NLS-1$
						+ req.getRequestURI());
		try {
			rd.forward(req, resp);
	...
	public void setHttpService(HttpService hs) {
		...
		try {
			...
			httpService
					.registerServlet(
							"/login", new FormOpenIdLoginServlet(this), null, httpContext); //$NON-NLS-1$
			httpService
					.registerServlet(
							"/logout", new FormOpenIdLogoutServlet(), null, httpContext); //$NON-NLS-1$
		} catch (ServletException e) {
			LogHelper.log(new Status(IStatus.ERROR,
					Activator.PI_FORMOPENID_SERVLETS, 1,
